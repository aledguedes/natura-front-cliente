...
export class EfetivarpedidoComponent implements OnInit {
  ...
  public buscarCPF() {
    ...
    this.cliService.buscarClientePeloCPF(this.cliente.cpf)
      .subscribe((cli: Cliente) => {
        this.cliente = cli;
        this.achou = true;
        this.msgEndereco = cli.logradouro.substring(0, 6) + " *********** ";
  ...
  public buscarCEP() {
    this.cepService.buscarCep(this.cliente.cep).subscribe
      ((res: EnderecoCep) => {
        ...
        (err) => {
          console.log("")
          this.mensagemErro = "Informe um CEP correto, sem pontos nem traços";
          document.getElementById("btnModal").click();
        });
  }
  ...
  public finalizarPedido() {
    ...
    pedidoTmp = JSON.parse(localStorage.getItem("shoopingCart"));
    ...
    this.pedService.inserirNovoPedido(this.pedido).subscribe(
      (res: Pedido) => {
        ...
        localStorage.removeItem("shoopingCart");
